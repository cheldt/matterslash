defmodule OpenWeatherMap.CurrentWeatherDataTest do
  ...
  @mapped_struct %OpenWeatherMap.CurrentWeatherData{
    country:     "DE",
  ...
  test "map_response_to_struct maps response data to struct and converts values to elixir types" do
    assert @mapped_struct === map_response_to_struct(@response, "metric", Application.get_env(:mattslasher, :openweathermap_api_units, %{}))
  end
  ...
  test "map_struct_to_list maps weather data to list and converts types to human readable format" do
    ...
    assert expected_list === map_struct_to_list(@mapped_struct, [["foo", "bar"]])
  end
end
